...
export default function PorschePage() {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/products/porsche");
        if (!res.ok) {
        ...
        setError("Failed to load Porsche models. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCarClick = (id: number) => {
    router.push(`/dashboard/customers/porsche/detail?id=${id}`);
  };
  ...
  return (
    <main className="min-h-screen p-8 bg-gradient-to-b from-white to-teal-100">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
        <h1 className="text-3xl font-bold">Porsche Models</h1>
        <input
          type="text"
          placeholder="Search car name..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full md:w-1/3 px-4 py-2 border rounded-full focus:outline-none focus:ring-2 focus:ring-black"
        />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-4 border rounded-xl shadow-sm bg-gray-50">
            <p className="text-gray-700 italic">
              "Driving a Porsche was always a dream. The Boxster made it real."
            </p>
